...
export default function AdVault() {
  ...
  return (
    <div style={{fontFamily: 'sans-serif', background: '#f5f5f5', minHeight: '100vh', paddingBottom:'50px'}}>
      ...
      {/* DETAIL MODAL (CASE STUDY - NO CAROUSEL) */}
        ...
        <div style={{position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.9)', zIndex:9999, overflowY:'auto', padding:'40px 0'}}>
          <div style={{maxWidth: '800px', margin: '0 auto', background: 'white', borderRadius: '15px', overflow:'hidden', minHeight:'90vh', boxShadow: '0 20px 50px rgba(0,0,0,0.5)', position:'relative'}}>
            ...
            <button 
              ...
              style={{position: 'absolute', top: '20px', right: '20px', zIndex: 10, background: '#eee', color: 'black', border: 'none', borderRadius: '50%', width: '40px', height: '40px', cursor: 'pointer', fontSize: '20px'}}
            >✕</button>
            ...
            <div style={{padding: '50px 40px'}}>
               ...
               <div style={{textAlign: 'center', marginBottom: '40px'}}>
                 ...
                 {activeCampaign.slogan && (
                   <div style={{fontSize: '24px', fontStyle: 'italic', fontFamily:'serif', color: '#111', marginBottom:'15px'}}>
                     "{activeCampaign.slogan}"
                   </div>
                 )}

                 {/* 3. METADATA ROW (THE PROMINENT DATA YOU ASKED FOR) */}
               ...
               {activeCampaign.image_urls && activeCampaign.image_urls.length > 0 && (
                 <div>
                   ...
                   <div style={{display: 'flex', flexDirection: 'column', gap: '20px'}}>
                     {activeCampaign.image_urls.map((img, i) => (
                       <img key={i} src={img} style={{width: '100%', borderRadius: '8px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)'}} />
                     ))}
                   </div>
                 </div>
               )}
          ...
          {step === 'review' && analysis && (
            <div>
              <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '20px'}}>
                ...
                <input value={analysis.format || ''} onChange={e => setAnalysis
